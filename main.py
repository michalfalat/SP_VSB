...
def projectArucoPosition(floor, marker):
    ...
    if(marker != None):
        ...
        angleRadians = (m.pi / 180.0) * (marker.estimatedAngle + markerPos.rotation)
        posPointX = int(middlePoint[0] + m.cos(angleRadians) * floor.calculateSizeToPixels(marker.estimatedDistance))
        posPointY = int(middlePoint[1] + m.sin(angleRadians) * floor.calculateSizeToPixels(marker.estimatedDistance))
